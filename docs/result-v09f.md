...
### PR の CI で、さらに 40em → 41em が出た

手元では 27 工程が緑でしたが、Linux の CI では `max-w-prose` を付けた
英語の段落だけが 41em になりました。こちらの本文幅 `--width-prose: 40em` と
Tailwind の `max-w-prose` は同じ名前に見えて、後者は標準の **65ch** のまま
だったためです。`ch` の実寸はフォントによって変わります。

`--max-width-prose` を `--width-prose` へ接続し、両方が同じ 40em を見る形に
しました。回帰判定も、表示幅ではなく **2 つの定義が同じ実寸か**を見ます。
