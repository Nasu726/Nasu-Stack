...
## Phase 5 — 利用者からの報告 7 件

作者が実際に触って挙げてくれたものです。**全部実在しました。**
うち 2 件は、検査を足しても最初は捕まえられませんでした。

### 1. `baseUrl` の非推奨警告

> オプション baseUrl は非推奨であり、TypeScript 7.0 で機能しなくなります。

**生成物を開いた瞬間にエディタが警告を出す。** 初めての人には「壊れている」と映ります。

`paths` の値を `./` で始めれば `baseUrl` は要りません。
生成物・カタログ・Astro サイト・検査用の雛型、4 か所すべてから外しました。
判定も足しています（`tsconfig に baseUrl が無い`）。
...
器（main）の右端   1465px
見出し h1 の右端   1449px
本文 p / ol の右端   947px   ← 500px の空白が残る
```

器を `width="content"`（広め）にして、**中の本文にだけ `max-w-prose` を
付けていました。** 見出しは器いっぱいに伸び、本文だけ半分で止まります。

**幅を決める柱は 1 本にします。** 器を `width="prose"`（40em）にし、
子の `max-w-*` を全部外しました。見出しと本文の右端が一致します（ずれ 0px）。

直した直後、既存の「1 行が長すぎる」判定が赤くなりました。
40em の器に 14px の文字を流すと 45.7em になり、和文の上限 45em を超えます。
**文字を小さくすると、必要な器も狭くなります。** 本文を 16px に揃えて解決しました。

判定 13 を足しました。**端末幅の検査は 1024px までしか見ません。**
広い画面でしか出ない崩れは、そこには映りません。

### 3. 生成直後の案内が流れて消える

> 操作している間に流れてしまうので良くない

そのとおりでした。画面に出すのは最初の 3 コマンドと「どこを読むか」だけにし、
中身は **`HowToUse.md`** に書き出します（何をどの順で触るか / 見た目の変え方 /
部品の足し方 / フォームの送信先 / 崩れの測り方 / 公開のしかた / 困ったとき）。

### 4. `npm run check` が動かない

2 つ重なっていました。

**a. パスが実体とずれていた。** `scripts/check-responsive.mjs` を指していましたが、
実体は `src/scripts/` です。ビルドも型検査も通るので、**既存の検査は全部緑**でした。
`package.json` の `scripts` が実在するファイルを指しているかを判定に足しました。

**b. playwright が入っていない。** 生の `ERR_MODULE_NOT_FOUND` が出ていました。
依存には入れません（実ブラウザは 100MB 超で、サイトを作るのに必須ではない）。
代わりに**入れ方を印字**します。スタックだけ見せても何をすればいいか分かりません。

### 5. まだ公開していないので部品を足せない

`components.json` は公開先の URL を指しています。マージするまで 404 です。
**仕様どおりで、欠陥ではありません。** 公開前に試す手順を回答に添えました。

### 6. 文章が初心者向けでなかった

> 「JavaScript は 1 バイトも読み込んでいません」とかは初心者向けではありません

対象は「多少プログラミングはできるが、**Web は初めて**」の人です。
island も SSG も知りません。用語を消し、**何が嬉しいのか**に書き換えました。

```
- このページは静的な HTML です。JavaScript は 1 バイトも読み込んでいません。
  動く部分が必要になったら、その部分だけ React の island にしてください。

+ このページは、あらかじめ組み立て済みの HTML をそのまま返しています。
+ ブラウザは受け取ってすぐ表示できるので、開くのが速く、検索エンジンにも
+ 読まれやすくなります。
```

### 7. 成功したボタンが hover で消える

`ActionButton` は成功時に `bg-success text-success-fg` を足していましたが、
**variant 側の `hover:bg-muted` を打ち消していませんでした。**

実測: 成功直後にポインタが乗ると背景が `oklch(0.968)`（薄い灰）になり、
文字は `text-success-fg`（ほぼ白）のまま。**読めません。**

**押したあと、マウスはボタンの上に残ります。** つまり成功の見た目は、
ほぼ必ず hover と同時に出ます。滅多に起きない組み合わせではなく、既定の経路でした。

`hover:bg-success hover:text-success-fg` を足して固定しました。

#### 判定を足しても、最初は捕まえられなかった

判定を書いてわざと壊したら、**緑のまま通りました。**

カタログの「成功する」ボタンは既定（primary）で、その hover は
`brightness-110`——**フィルタなので背景色そのものは変わりません。**
壊れていても計測値は緑のままです。

背景を差し替える hover を持つ `outline` のボタンをカタログに 1 つ足し、
そこを見るようにして、ようやく赤くなりました
（実測 `oklch(0.968)` / 期待 `oklch(0.55)`）。

**「検査は通っている」と「検査対象に入っている」は別のこと**——
v0.5.5 で一度書いた教訓に、同じ形で再会しました。

### 数字

| | 前 | 後 |
|---|---|---|
| `pnpm verify` | 20 工程 | 20 工程（判定は増加） |
| `pnpm verify:create` | 38 判定 | **47 判定** |

---

## Phase 6 — 依存の更新で見えたもの

利用者からの報告 5 件。うち**確実な欠陥が 1 件、判明した設計の誤りが 2 件**でした。

### 1. 配っている astro が 2 メジャー遅れで、XSS の勧告を 8 件抱えていた

生成物で `npm install` すると出ます。

```
3 vulnerabilities (1 low, 2 high)
New version of Astro available: 7.2.2
```

「新しい版があります」の通知ではありません。**配っている 5.18.2 に
XSS / SSRF の勧告が 8 件**ありました。利用者は install した瞬間に
それを見ますが、こちらは何も知らないままです。

`security.md` §6 に「Renovate をまだ動かしていない。固定は勝手に直らない」と
書いたその状態が、そのまま現実になりました。**固定と追随は対で持つものです。**

上げた結果: astro 7.2.2 / @astrojs/react 6.0.2 / vite 8.2.1 /
@vitejs/plugin-react 6.0.5。生成物の**脆弱性 0 件**、パッケージ数 411 → 350。

判定 8.2 を足しました（`pnpm audit --audit-level high --prod`）。
届かない環境では理由を印字して飛ばします。

### 2. `::backdrop` の判定が、色の**表記**を見ていた

更新後、無関係な 2 工程が赤くなりました。

```
✗ ::backdrop に色が付く  (oklch(0 0 0 / 0.45))
```

値は正しく、背景も出ています。判定が `/rgba?\(/` で、
「ブラウザが rgb() の形で返す」という実装の都合に寄りかかっていました。
Vite の版が上がって出力が oklch() のまま届いた瞬間、**部品は無事なのに
検査だけが落ちました。**

「透明でないこと」だけを見る `isPainted()` にして、`_browser.mjs` へ寄せました。
**直すところが無いのに赤い**のは、いちばん質の悪い赤です。
放っておくと「また表記の話だろう」と読み飛ばされます。

### 3. `astro preview` がデーモンになっていた（土台の作り直し）

astro 7 で挙動が変わりました。

```
Preview server already running at http://127.0.0.1:4322 (pid 28392)
  Stop: astro preview stop
```

- 自分を子プロセスとして起動し直し、**親はすぐ終了する**
- ポートが埋まっていると**黙って別の番号へ逃げる**
- 止めるには `astro preview stop` という別の口が要る

検査の土台は「指定したポートで前面に居るサーバ」を前提にしていたので、
**実ブラウザの 8 工程がまとめて「接続できません」**になりました。しかも
握っている PID は既に死んでいるので止められず、残骸が次の実行で
「消えたディレクトリを配るサーバ」として判定に当たりました。

道具の preview を追いかけるのをやめました。見たいのは
「ビルドした中身が正しく出るか」であって「その道具の preview が動くか」では
ありません。`scripts/_static.mjs` に素の静的サーバを 1 つ置き、
`verify` も `verify-create` も `serve-registry` もそこを使います。
**動く部品が 1 つ減りました。**

代わりに失ったもの: 利用者が打つ `npm run preview` そのものは検査しません。

#### 途中で 1 回間違えました

最初、サーバを `verify.mjs` と**同じプロセス**に置きました。動きません。
`step()` は `spawnSync`（同期）なので、**子が走っている間はイベントループが
止まり、要求に応答できません。** 接続は受け付けるが返事が来ないので、
検査側は `networkidle` を 30 秒待って諦めます。

「落ちている」のではなく「返事ができない」——これも分かりにくい壊れ方でした。
`scripts/serve-static.mjs` として別プロセスに出して解決しています。

### 4. サーバが立たなくても、黙って先へ進んでいた

上の切り分けに時間を取られた理由がこれです。起動を待つループは
タイムアウトしても**何も言わずに次へ行っていました。**
名指しで 1 工程落とすようにしました。

ポートが埋まっているときも同じで、`_static.mjs` は
EADDRINUSE を「前の実行が残したサーバを止めてください」と、
止め方のコマンド付きで返します。

### 5. 報告のうち、欠陥ではなかったもの

- **`node_modules/astro/*.d.ts` が見つからない**（エディタの表示）。
  新規生成 + install した状態では `astro check` が 0 エラーで、
  当該ファイルも実在します。Vite 版に astro は入りません。
  **VS Code を開き直したら消えた**とのことで、エディタが古いプログラムを
  掴んでいたものと判断します
- **未公開なので部品を足せない。** 仕様どおりです

### 積み残し（利用者からの要望）

| | 内容 | 扱い |
|---|---|---|
| 補完とスニペット | 部品の目印と、属性を埋めた雛型の補完 | **v0.9b の本命候補。** `.vscode/*.code-snippets` を registry から生成する |
| 雛型を増やす | ブログ付きサイト / LP / ダッシュボード | v0.9b。**1 つ足すごとに実 install・build・実ブラウザが乗る**ので、検査の重さも一緒に設計する |
| GUI ナビゲータ | FastAPI の docs のようなもの | 後回しで合意。作るなら別パッケージにして本体を汚さない |
