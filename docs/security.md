...
- **The SHA-256 of the distributed tarball is published alongside it**
  (`create-nasu-stack-2.2.0.tgz.sha256`), so you can compare before you run
  anything
- **Every URL is https** — the registry JSON and the tarball alike

```bash
# For v2.2.0, download once, verify that file, and run that same file.
...
sha256sum -c create-nasu-stack-2.2.0.tgz.sha256
