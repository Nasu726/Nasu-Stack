...
```bash
pnpm verify   # 型検査・ビルド・配布物・実ブラウザ検証をまとめて実行（34 工程）
...
✓ 単体: useResource の query key       ✓ 実ブラウザ: テーマの配色と選択操作
