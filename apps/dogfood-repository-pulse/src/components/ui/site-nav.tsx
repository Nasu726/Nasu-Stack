 ...
 * 重なりの順序（z-index）
 * ----------------------------------------------------------------
 * ヘッダは **`z-30`** です。`sticky` で貼り付いたまま、下の中身の上に
 * 出る必要があるためです。
 *
 * **これより手前に出したいものがあるときは、そちら側を上げてください。**
 * `<SiteHeader>` をページの一部として埋め込むと（部品のカタログや
 * 管理画面のプレビュー枠など）、外側の枠が `z-30` 未満だと
 * **ヘッダのほうが手前に出ます。**
 *
 * ```tsx
 * <header className="sticky top-0 z-40">…</header>   // 外側の枠
 * <SiteHeader sticky={false} />                      // 中に置くほうは貼り付けない
 * ```
 *
 * ヘッダ自身を下げたいときは `className="z-10"` で上書きできます
 * （`cn` が後勝ちで解決します）。
 *
 * ----------------------------------------------------------------
 * リンクは両方の並びに出しています
