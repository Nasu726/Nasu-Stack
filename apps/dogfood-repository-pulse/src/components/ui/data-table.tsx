...
export function DataTable<T>({
  ...
  if (selectable && !getKey && typeof console !== "undefined") {
    // 型で強制できないので実行時に知らせます
    console.warn(
      "[DataTable] getKey is required when selectable is enabled. " +
        "An index key can move selection to a different row after sorting.",
    );
  }
      ...
      {caption && (
        <Spread space="sm">
          ...
          {state.status === "success" && (
            <span className="text-xs text-muted-fg">{total} items</span>
          )}
        </Spread>
      )}
        ...
        {(data) => (
          <>
            ...
            <div className={mobile === "cards" ? "hidden md:block" : "block"}>
              <Scrollable label={typeof caption === "string" ? caption : "Table"}>
                ...
                {selectable && (
                  <Inline space="xs" alignY="center">
                    <SelectAllBox
                      ...
                      selection={selection}
                      onChange={(checked) => toggleAllOnPage(data, checked)}
                      withLabel="Select all visible rows"
                    />
                  </Inline>
                )}
                <SortBar
...
function SelectAllBox({
  ...
  const box = (
    <input
      ...
      aria-label={withLabel ?? "Select all visible rows"}
