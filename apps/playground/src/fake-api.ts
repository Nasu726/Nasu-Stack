...
const TASKS: Task[] = [
  { id: 1, title: t("ヒーローセクションの文言を確定する"), owner: "me", done: true },
  { id: 2, title: t("問い合わせフォームを API に接続"), owner: "me", done: false },
  { id: 3, title: t("OGP 画像の生成を自動化"), owner: "collaborator", done: false },
  { id: 4, title: t("Lighthouse のスコアを 95 以上にする"), owner: "me", done: false },
];
...
export async function alwaysFail(_: void, ctx: { signal: AbortSignal }) {
  ...
  throw new ActionError("upstream 503", {
    displayMessage: t("サーバーが混み合っています。少し待って再実行してください。"),
    code: 503,
  });
}
...
export async function signup(
  ...
  if (!name.trim()) fields.name = t("お名前を入力してください");
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email))
    fields.email = t("メールアドレスの形式が正しくありません");
  if (password.length < 8)
    fields.password = t("パスワードは 8 文字以上にしてください");
  if (email.endsWith("@example.com"))
    fields.email = t("このドメインは登録できません");
