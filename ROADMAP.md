...
ナビ・開閉・本文・画像が揃ったので、ページ雛型が「組み合わせ」として安く作れます。
**この順にしたのは、部品が無い状態で雛型を作ると生の Tailwind の手書きが増えるだけだから**です
（実際、v0.6 の前の `index.astro` は生の `class=""` が 19 箇所ありました）。

- [ ] ランディング / ブログ / 会社サイト / ダッシュボードの雛型
- [ ] 各雛型を registry のアイテムとして配布
- [ ] Astro 版と React 版の両方
- [ ] SEO / OGP / sitemap / RSS（雛型と同時に入れると決めました）
