...
| `https://github.com/Nasu726/Nasu-Stack/releases/download/v2.2.0/create-nasu-stack-2.2.0.tgz` | version付きStable入口。workflowは上書きしない |
...
sha256sum -c create-nasu-stack-2.2.0.tgz.sha256   # OK と出たら
npx ./create-nasu-stack-2.2.0.tgz my-site
```

（Windows の PowerShell なら
`Get-FileHash create-nasu-stack-2.2.0.tgz -Algorithm SHA256` で出した値を、
`.sha256` の中身と見比べてください。）
